...
function getEmptyInventory(): Inventory {
  const items = Object.fromEntries(PRODUCTS.map((p) => [p.key, ""])) as Inventory["items"];
  return { items };
}
...
export async function getInventory(): Promise<Inventory> {
  ...
  const data = snap.data() as { items?: Record<string, string> };
  const items = getEmptyInventory().items;

  for (const p of PRODUCTS) {
    const val = data.items?.[p.key];
    items[p.key] = typeof val === "string" ? val : "";
  }
